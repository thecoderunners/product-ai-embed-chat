(function(){"use strict";({})?Promise.resolve().then(()=>k).then(p=>{const o=p.default,d=document.createElement("style");d.textContent=o,document.head.appendChild(d)}):fetch("/src/styles.css").then(p=>p.text()).then(p=>{const o=document.createElement("style");o.textContent=p,document.head.appendChild(o)}).catch(p=>console.warn("Could not load styles:",p)),function(){function p(){const t=document.currentScript;if(t&&t.getAttribute("data-api-key"))return t.getAttribute("data-api-key");const r=document.querySelectorAll('script[type="module"]');for(const a of r)if(a.getAttribute("data-api-key")&&(a.src.includes("chat-widget")||a.src.includes("script.js")))return a.getAttribute("data-api-key");const e=document.querySelector("script[data-api-key]");return e?e.getAttribute("data-api-key"):(console.warn("Could not find API key from script tag"),null)}if(!document.querySelector('meta[name="viewport"]')){const t=document.createElement("meta");t.name="viewport",t.content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no",document.head.appendChild(t)}const o={info:(...t)=>console.log("📱 [Chat Widget]:",...t),error:(...t)=>console.error("❌ [Chat Widget]:",...t),warn:(...t)=>console.warn("⚠️ [Chat Widget]:",...t)};o.info("Initializing chat widget...");const d=document.createElement("div");d.id="my-chat-widget",d.style.position="fixed",d.style.bottom="20px",d.style.right="20px",d.style.zIndex="10000",document.body.appendChild(d),o.info("Chat container created");const g=document.createElement("button");g.className="chat-toggle",g.innerHTML=`
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
    </svg>
  `,d.appendChild(g);const l=document.createElement("div");l.id="my-chat-window",d.appendChild(l),o.info("Chat button and window created"),p();const z=p();let h=!1;function x(){return window.innerWidth<768}function b(){o.info("Toggling chat window, current state:",h),h?(l.classList.remove("chat-show"),l.classList.add("chat-hide"),l.addEventListener("animationend",()=>{l.style.display="none",h=!1,document.documentElement.classList.remove("chat-open"),document.body.classList.remove("chat-open"),document.body.style.overflow=""},{once:!0})):(l.style.display="block",l.classList.add("chat-show"),l.classList.remove("chat-hide"),h=!0,x()&&(document.documentElement.classList.add("chat-open"),document.body.classList.add("chat-open"),document.body.style.overflow="hidden",setTimeout(()=>{window.scrollTo(0,0)},10)),S())}g.addEventListener("click",b),window.addEventListener("resize",()=>{if(h){const t=document.getElementById("chat-body");t&&(t.style.height=x()?"calc(100% - 120px)":"calc(100% - clamp(120px, 25vh, 140px))")}});function S(){o.info("Loading chat interface..."),$()}function $(){l.innerHTML=`
      <div id="chat-header">
        <span>Chat</span>
    ...
    `,l.querySelector(".close-button").addEventListener("click",r=>{r.stopPropagation(),b()}),f({type:"text",content:"Hi! How can I help you today?"}),I(),o.info("Chat interface loaded and initialized")}function f(t){const r=document.getElementById("chat-body"),e=document.createElement("div");switch(e.className="message server-message",t.type){case"text":e.innerHTML=`
            <div class="message-content">
              ${t.content}
            </div>
          `;break;case"product":const i=t.originalPrice?Math.round((t.originalPrice-t.price)/t.originalPrice*100):0,n=t.rating?E(t.rating):"";e.innerHTML=`
            <div class="product-card">
              ...
              <div class="product-actions">
                ${t.actions.map(c=>`
                  ${c.url?`<a href="${c.url}" class="product-action-button" data-action="${c.value}">${c.label}</a>`:`<button class="product-action-button" data-action="${c.value}">${c.label}</button>`}
                `).join("")}
              </div>
            </div>
          ...
          <div class="message-content">
            ...
            <div class="action-buttons">
              ${t.options.map(c=>`<button class="action-button" data-action="${c.value}">${c.label}</button>`).join("")}
            </div>
          </div>
