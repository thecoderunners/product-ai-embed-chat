(function(){"use strict";Promise.resolve().then(()=>S).then(m=>{const e=m.default,d=document.createElement("style");d.textContent=e,document.head.appendChild(d)}),function(){function m(){const t=document.currentScript;if(t&&t.getAttribute("data-api-key"))return t.getAttribute("data-api-key");const i=document.querySelectorAll('script[type="module"]');for(const c of i)if(c.getAttribute("data-api-key")&&(c.src.includes("chat-widget")||c.src.includes("script.js")))return c.getAttribute("data-api-key");const o=document.querySelector("script[data-api-key]");return o?o.getAttribute("data-api-key"):(console.warn("Could not find API key from script tag"),null)}if(!document.querySelector('meta[name="viewport"]')){const t=document.createElement("meta");t.name="viewport",t.content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no",document.head.appendChild(t)}const e={info:(...t)=>console.log("📱 [Chat Widget]:",...t),error:(...t)=>console.error("❌ [Chat Widget]:",...t),warn:(...t)=>console.warn("⚠️ [Chat Widget]:",...t)};e.info("Initializing chat widget...");const d=document.createElement("div");d.id="my-chat-widget",d.style.position="fixed",d.style.bottom="20px",d.style.right="20px",d.style.zIndex="10000",document.body.appendChild(d),e.info("Chat container created");const g=document.createElement("button");g.className="chat-toggle",g.innerHTML=`
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
    </svg>
  `,d.appendChild(g);const s=document.createElement("div");s.id="my-chat-window",d.appendChild(s),e.info("Chat button and window created");let h=null;const E=m();function $(){return e.info("Requesting auth token..."),fetch("http://localhost:3000/api/token",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json","X-Api-Key":E}}).then(t=>t.json()).then(t=>{t.success&&t.data.token?(h=t.data.token,e.info("Received auth token:",h)):e.error("Failed to retrieve auth token:",t.error)}).catch(t=>{e.error("Error fetching auth token:",t)})}let u=!1;function b(){return window.innerWidth<768}function w(){e.info("Toggling chat window, current state:",u),u?(s.classList.remove("chat-show"),s.classList.add("chat-hide"),s.addEventListener("animationend",()=>{s.style.display="none",u=!1,document.documentElement.classList.remove("chat-open"),document.body.classList.remove("chat-open"),document.body.style.overflow=""},{once:!0})):(s.style.display="block",s.classList.add("chat-show"),s.classList.remove("chat-hide"),u=!0,b()&&(document.documentElement.classList.add("chat-open"),document.body.classList.add("chat-open"),document.body.style.overflow="hidden",setTimeout(()=>{window.scrollTo(0,0)},10)),C())}g.addEventListener("click",w),window.addEventListener("resize",()=>{if(u){const t=document.getElementById("chat-body");t&&(t.style.height=b()?"calc(100% - 120px)":"calc(100% - clamp(120px, 25vh, 140px))")}});function C(){e.info("Loading chat interface..."),h?v():$().then(()=>{h&&v()})}function v(){fetch("http://localhost:3000/api/chat/init",{method:"GET",credentials:"include",headers:{Authorization:`Bearer ${h}`}}).then(t=>(e.info("Chat init API response received"),t.json())).then(t=>{s.innerHTML=`
          <div id="chat-header">
            <span>Chat</span>
        ...
        `,s.querySelector(".close-button").addEventListener("click",o=>{o.stopPropagation(),w()}),t.messages&&Array.isArray(t.messages)?t.messages.forEach(o=>{f(o)}):f({type:"text",content:"Hi! How can I help you today?"}),B(),e.info("Chat interface loaded and initialized")}).catch(t=>{e.error("Error loading chat interface:",t),s.innerHTML=`
        ...
        `})}function f(t){const i=document.getElementById("chat-body"),o=document.createElement("div");switch(o.className="message server-message",t.type){case"text":o.innerHTML=`
            <div class="message-content">
              ${t.content}
            </div>
          `;break;case"product":const a=t.originalPrice?Math.round((t.originalPrice-t.price)/t.originalPrice*100):0,n=t.rating?L(t.rating):"";o.innerHTML=`
            <div class="product-card">
              ...
              <div class="product-details">
                ...
                <div class="product-price">
                  <strong>${y(t.price)}</strong>
                  ${t.originalPrice?`<span class="original-price">${y(t.originalPrice)}</span>`:""}
                </div>
