(function(){"use strict";Promise.resolve().then(()=>y).then(o=>{const d=o.default,h=document.createElement("style");h.textContent=d,document.head.appendChild(h)}),function(){if(!document.querySelector('meta[name="viewport"]')){const t=document.createElement("meta");t.name="viewport",t.content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no",document.head.appendChild(t)}const o={info:(...t)=>console.log("📱 [Chat Widget]:",...t),error:(...t)=>console.error("❌ [Chat Widget]:",...t),warn:(...t)=>console.warn("⚠️ [Chat Widget]:",...t)};o.info("Initializing chat widget...");const d=document.createElement("div");d.id="my-chat-widget",d.style.position="fixed",d.style.bottom="20px",d.style.right="20px",d.style.zIndex="10000",document.body.appendChild(d),o.info("Chat container created");const h=document.createElement("button");h.className="chat-toggle",h.innerHTML=`
  ...
  `,d.appendChild(h);const c=document.createElement("div");c.id="my-chat-window",d.appendChild(c),o.info("Chat button and window created");let u=!1;function m(){return window.innerWidth<768}function x(){o.info("Toggling chat window, current state:",u),u?(c.classList.remove("chat-show"),c.classList.add("chat-hide"),c.addEventListener("animationend",()=>{c.style.display="none",u=!1,document.documentElement.classList.remove("chat-open"),document.body.classList.remove("chat-open"),document.body.style.overflow=""},{once:!0})):(c.style.display="block",c.classList.add("chat-show"),c.classList.remove("chat-hide"),u=!0,m()&&(document.documentElement.classList.add("chat-open"),document.body.classList.add("chat-open"),document.body.style.overflow="hidden",setTimeout(()=>{window.scrollTo(0,0)},10)),k())}h.addEventListener("click",x),window.addEventListener("resize",()=>{if(u){const t=document.getElementById("chat-body");t&&(t.style.height=m()?"calc(100% - 120px)":"calc(100% - clamp(120px, 25vh, 140px))")}});function k(){o.info("Loading chat interface..."),fetch("http://localhost:3000/api/chat/init",{method:"GET",credentials:"include"}).then(t=>(o.info("Chat init API response received"),t.json())).then(t=>{c.innerHTML=`
        ...
        `,c.querySelector(".close-button").addEventListener("click",e=>{e.stopPropagation(),x()}),t.messages&&Array.isArray(t.messages)?t.messages.forEach(e=>{g(e)}):g({type:"text",content:"Hi! How can I help you today?"}),E(),o.info("Chat interface loaded and initialized")}).catch(t=>{o.error("Error loading chat interface:",t),c.innerHTML=`
        ...
        `})}function g(t){const i=document.getElementById("chat-body"),e=document.createElement("div");switch(e.className="message server-message",t.type){case"text":e.innerHTML=`
          ...
          `;break;case"product":const a=t.originalPrice?Math.round((t.originalPrice-t.price)/t.originalPrice*100):0,n=t.rating?z(t.rating):"";e.innerHTML=`
          ...
          <div class="message-content">
            ...
            <div class="action-buttons">
              ${t.options.map(s=>`<button class="action-button" data-action="${s.value}">${s.label}</button>`).join("")}
            </div>
          </div>
